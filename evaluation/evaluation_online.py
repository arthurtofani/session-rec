...
def evaluate_sessions(pr, metrics, test_data, train_data, items=None, num_predicts=20, training_sessions=0, session_key='SessionId', item_key='ItemId', time_key='Time', num_sessions=1000):
    '''
    ...
    test_data = train_data.append(test_data).sort_values('SessionId')
    actions = len(test_data)
    sessions = len(test_data[session_key].unique())
    count = 0
    ...
    sessions_train = test_data.loc[test_data[session_key] == -1]
    tracks_set = np.array([])

    # TRAIN TFIDF WITH TRAIN DATA
    reward = 0
    step = 0

    pr.fit(train_data)
    sessions_df = test_data.groupby([session_key])
    total_steps = min(num_sessions, len(sessions_df))
    pbar = tqdm(sessions_df, total=total_steps)
    for sid, session in pbar:
        session_result = {'sid': sid, 'num_items': session.shape[0]}
        if (session.shape[0]) > 5:
            tracks = session[item_key].values
            times = session[time_key].values
            reward_s = 0
            matches = 0
            t0 = time.time()
            freq_pos = defaultdict(lambda: 0)
            for i in range(len(tracks)-1):
                curr_item = tracks[i]
                next_item = tracks[i + 1]
                preds = pr.predict_next(sid, curr_item, tracks_set, timestamp=times[i+1])
                try:
                    if preds is not None:
                        prd = preds.sort_values(ascending=False)
                        pos = list(prd.index.values[:num_predicts]).index(next_item)
                        if pos <= num_predicts:
                            freq_pos[pos] += 1
                        reward_s += 1 / (1 + pos)
                        matches += 1
                    ...
                    pass
            #import code; code.interact(local=dict(globals(), **locals()))
            session_result['predict_time'] = time.time() - t0
            session_result['reward'] = reward_s
            session_result['reward_norm'] = reward_s / (len(tracks) - 1)
            session_result['matches'] = matches
            session_result['pos_0'] = freq_pos[0]
            session_result['pos_1'] = freq_pos[1]
            session_result['pos_2'] = freq_pos[2]
            session_result['pos_3'] = freq_pos[3]
            reward += session_result['reward_norm']
            pbar.set_postfix({'reward': round(reward, 2)})

        sessions_train = pd.concat([sessions_train, session])
        tracks_set = sessions_train[item_key].unique()
        t0 = time.time()
        pr.fit(session)
        session_result['fit_time'] = time.time() - t0
        results.append(session_result)
        # TIME SPENT IN ONE SESSION
        step +=1
        #if (step+1) % 500 == 0:
        #    print(reward)
        if step >= total_steps:
            break
        #if (step+1) % 100 == 0:
        #    np.save("results/markov_reward_x.npy", acc_reward)
        #    np.save("results/markov_time_spent_x.npy", acc_time)
